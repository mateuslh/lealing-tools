...
import (
	...
	"github.com/mateuslh/lealing-tools/internal/systeminfo/macos"
	ui "github.com/mateuslh/lealing-tools/internal/systeminfo/ui"
...
var modelFactory screen.Factory = func(session screen.Session) screen.Model {
	...
	if session.Initialize.Platform == "windows" {
		return ui.New(deps, windows.NewSystemInspector(nil), nil)
	}
	return ui.New(deps, macos.NewSystemInspector(nil), nil)
}
...
func main() {
	...
	if err := runProtocol(ctx, os.Stdin, os.Stdout); err != nil {
		fmt.Fprintln(os.Stderr, "system-info:", err)
		os.Exit(1)
	}
}
